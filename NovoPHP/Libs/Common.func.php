...
**/
function getRealIpAddressForNginx(){
 ...
 */
function getRealIpAddressForApache($phpMode=1)
{
 ...
 * @return 排序后的Array。
 **/
function array_sort($a,$sort,$d='asc')
{
    $num=count($a);
    if($d == 'asc'){
        for($i=0;$i<$num;$i++){
 ...
 * @return 62进制字符串
 **/
function hash10To62($hashString) {
 ...
 * @return 10进制字符串
 **/
function hash62To10($hashString) {
 ...
 * @return 62进制字符串
 **/
function hash16To62($hashString) {
 ...
 * @return 16进制字符串
 **/
function hash62To16($hashString) {
 ...
 */
function packageReplaceString($encryptString)
{
 ...
 */
function unPackageReplaceString($inString)
{
    return preg_replace_callback(
        '/(\w{2})/',
        function ($r){
 ...
 * @return 打包后的字符串
 **/
function packURIString($inString)
{
    $encryptString = substr(base64_encode(rand().time()),0,3).base64_encode(BaseStringEncrypt::xxteaEncrypt($inString, md5(ENCRYPT_PUB_KEY)));
    //return hash16To62(preg_replace('/(.)/es',"str_pad(dechex(ord('\\1')),2,'0',STR_PAD_LEFT)", $encryptString));
    return hash16To62(packageReplaceString($encryptString));
}
 ...
 * @return 解包出来的字符串
 **/
function unpackURIString($inString)
{
    //$inString = base64_decode(substr(preg_replace('/(\w{2})/e',"chr(hexdec('\\1'))", hash62To16($inString)), 3));
    $inString = base64_decode(substr(unPackageReplaceString($inString), 3));
    return BaseStringEncrypt::xxteaDecrypt($inString,md5(ENCRYPT_PUB_KEY));
}
...
{
    //$inString = base64_decode(substr(preg_replace('/(\w{2})/e',"chr(hexdec('\\1'))", hash62To16($inString)), 9));
    $inString = base64_decode(substr(unPackageReplaceString($inString), 9));
...
{
    $encode_str = substr(base64_encode(rand().time()),0,6).base64_encode(BaseStringEncrypt::xxteaEncrypt($str,md5(ENCRYPT_PUB_KEY)));
    return hash16To62(BaseStringEncrypt::disorganizeString(packageReplaceString($encode_str)));
}
...
{
    return BaseStringEncrypt::xxteaDecrypt(base64_decode(substr(preg_replace_callback('/(\w{2})/',function($r){return chr(hexdec($r[1]));},
        BaseStringEncrypt::unDisorganizeString(hash62To16($str))),6)),md5(ENCRYPT_PUB_KEY));
}
 ...
 * @return 打包后的字符串
 **/
function normalPackArray($inArray)
{
    $encode_str = BaseStringEncrypt::xxteaEncrypt(serialize($inArray),ENCRYPT_PUB_KEY);
    return BaseStringEncrypt::encryptString(substr(base64_encode(rand().time()),0,9).base64_encode($encode_str), ENCRYPT_PUB_KEY_BAK);
}
 ...
 * @return 解包出来的Array数组
 **/
function normalUnpackArray($inString)
{
    return unserialize(BaseStringEncrypt::xxteaDecrypt(base64_decode(substr(BaseStringEncrypt::decryptString($inString, ENCRYPT_PUB_KEY_BAK),9)),ENCRYPT_PUB_KEY));
}
 ...
 * @return 打包后的字符串
 **/
function iBase64Encode($inArray)
{
 ...
 * @return 解包出来的Array数组
 **/
function iBase64Decode($inString)
{
 ...
 * @return string date.
 */
function strTimeToDate($timeStr){
 ...
 */
function Text2HTML($contents, $showBRTag=true)
{
    $pre_tags = array();
 ...
 */
function rebuildURI($parameter, $escape_query_key = "")
{
